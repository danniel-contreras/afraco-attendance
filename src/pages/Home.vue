  ...
  <Layout title="Inicio">
    <div class="w-full h-full flex flex-col">
      <div
        @click="Loading"
        v-if="!loading"
        class="flex w-full cursor-pointer justify-center mt-6 items-center"
      >
        <OhVueIcon name="co-reload" />
        <p class="">Volver a verificar</p>
      </div>
      <div v-if="!loading" class="flex flex-col w-full h-full items-center">
        ...
        <template v-if="!showing">
          <button
            @click="Loading"
            class="w-64 md:w-64 lg:w-44 bg-blue-800 p-4 py-5 text-white text-lg font-semibold rounded-full"
          >
            Verificar
          </button>
        </template>
          ...
          <div v-else>
            <div class="flex flex-col">
              <button
                ...
                class="bg-blue-800 w-64 text-white p-4 py-5 rounded-full my-3"
              >
                Marcar entrada
              </button>
              <p v-else class="text-lg text-center pt-10">
                <span class="font-semibold">Hora entrada:</span>
                {{ attendance?.entryTime }}
              </p>
              <button
                v-if="!attendance?.exitState"
                @click="MarkAttendanceExit"
                :disabled="!attendance"
                class="bg-[#A20303] w-64 text-white p-4 py-5 rounded-full"
              >
                Marcar salida
              </button>
              <p v-else class="text-lg text-center pt-5">
...
const Loading = async () => {
  loading.value = true;
  VerifyAuth(employee.value.id_employee).then(() => {
    showing.value = true;
    loading.value = false;
  });
  await VerifyAttendance(
    employee.value.id_employee,
    format_date(datetime.value.datetime)
  );
};
...
const MarkAttendanceExit = async () => {
  ...
  if (attendance.value)
    await ExitAttendance(data, attendance.value.id, format_date(datetime.value.datetime));
};
</script>
